---
name: do
description: Execute a phased implementation plan using subagents. Use when asked to execute, run, or carry out a plan — especially one created by make-plan. Deploys fresh subagents per phase with verification gates between each phase.
metadata:
...
Works best when preceded by `make-plan` which produces the phased plan this skill executes. The plan from `make-plan` becomes the execution manifest for `do`.
